...
export class PDFExporter {
  ...
  private addHeader(personalInfo: ResumeData['personalInfo']): void {
    ...
    this.pdf.text(personalInfo.fullName || 'Your Name', this.pageWidth / 2, this.yPosition, { align: 'center' });
    this.yPosition += 10;
    ...
    if (contactInfo) {
      this.pdf.text(contactInfo, this.pageWidth / 2, this.yPosition, { align: 'center' });
      this.yPosition += 6;
    }
    ...
    if (webInfo) {
      this.pdf.text(webInfo, this.pageWidth / 2, this.yPosition, { align: 'center' });
      this.yPosition += 6;
    }

    // Add a line separator
    this.pdf.setLineWidth(0.5);
    this.pdf.line(this.margin, this.yPosition + 2, this.pageWidth - this.margin, this.yPosition + 2);
    this.yPosition += 8;
  }
  ...
  private addExperience(experience: ResumeData['experience']): void {
    ...
    experience.forEach((exp, index) => {
      ...
      this.pdf.text(dateRange, this.pageWidth - this.margin, this.yPosition, { align: 'right' });
      this.yPosition += 6;
